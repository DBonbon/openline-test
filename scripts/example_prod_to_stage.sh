...
readonly REMOTE_MEDIA_PATH=/mnt/persist/www/openline/shared/media
...
ssh $PROD_HOST "pg_dump -h localhost -Fc -f /tmp/db-dump.sql -U postgres openline -x -O"
...
ssh $STAGE_HOST "sudo -u postgres pg_restore --clean -h localhost -d openline -U postgres '/tmp/db-dump.sql'"
ssh $STAGE_HOST "rm /tmp/db-dump.sql"

rm /tmp/db-dump.sql

echo "Updating database..."
manage_prefix="source /mnt/persist/www/openline/shared/venv/bin/activate && cd /mnt/persist/www/openline/current/src &&"
